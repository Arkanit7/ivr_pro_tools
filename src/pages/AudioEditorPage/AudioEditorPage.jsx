...
function drawCanvas(
  ...
  for (let ch = 0; ch < numCh; ch++) {
    ...
    for (let x = 0; x < drawW; x++) {
      ...
      const s1 = Math.min(
        buf.length,
        Math.max(s0 + 1, Math.floor((x + 1) * spx)),
      )
      let mn = 0,
        mx = 0
      for (let i = s0; i < s1; i++) {
...
export default function AudioEditorPage() {
  ...
  const [status, setStatus] = useState(
    'Імпортуйте WAV або ALW файли або перетягніть їх сюди.',
  )
  ...
  const maxDur = clips.length
    ? Math.max(...clips.map((c) => c.buffer.duration))
    : 0
  ...
  const startPlayback = useCallback(() => {
    ...
    const focusedClip = cur.clipId
      ? clips.find((c) => c.id === cur.clipId)
      : null
    ...
    const tick = () => {
      ...
      playPosRef.current =
        ...
        Math.round(
          (ctx.currentTime - playStartRef.current) * TARGET_SAMPLE_RATE,
        )
      ...
      const px = Math.round(
        (localPh / Math.max(1, visSamplesRef.current)) *
  ...
  const handleCut = useCallback(() => {
    ...
    setSelection(null)
    setStatus(
      `Вирізано ${fmt((sel.end - sel.start) / TARGET_SAMPLE_RATE)} → буфер.`,
    )
  }, [pushUndo])
  ...
  const handleCopy = useCallback(() => {
    ...
    setClipboard(cb)
    setStatus(
      `Скопійовано ${fmt((sel.end - sel.start) / TARGET_SAMPLE_RATE)} до буфера.`,
    )
  }, [])
  ...
  const handleExportAlw = useCallback(async () => {
    ...
    try {
      ...
      URL.revokeObjectURL(url)
      setStatus(
        `Експортовано audio_export.alw — ${fmt(mono8k.duration)} @ 8 кГц моно`,
      )
    } catch (err) {
  ...
  const onMouseDown = useCallback((clipId, x, canvasW, visSamples) => {
    ...
    const sample = Math.max(
      0,
      Math.min(clip.buffer.length, Math.round((x / canvasW) * visSamples)),
    )
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        if (e.code === 'KeyX') {
          e.preventDefault()
          handlerRefs.current.handleCut?.()
        }
        if (e.code === 'KeyC') {
          e.preventDefault()
          handlerRefs.current.handleCopy?.()
        }
        if (e.code === 'KeyV') {
          e.preventDefault()
          handlerRefs.current.handlePaste?.()
        }
        if (e.code === 'KeyZ') {
          e.preventDefault()
          handlerRefs.current.handleUndo?.()
        }
        if (e.code === 'KeyA') {
          e.preventDefault()
          handlerRefs.current.handleSelectAll?.()
        }
      }
    }
        ...
        <CardContent className="mt-4 flex flex-col gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-1.5">
              <Label htmlFor="sil" className="text-xs whitespace-nowrap">
                Тиша (с):
              </Label>
              <Input
                id="sil"
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="w-9 text-center text-xs text-muted-foreground tabular-nums">
              ...
              >
                ...
                <p className="text-sm">
                  {isDraggingOver ? (
                    ...
                    <>
                      Перетягніть WAV або ALW файли сюди або натисніть{' '}
                ...
                {!isDraggingOver && (
                  <p className="text-xs opacity-60">
                    Кожен файл стає окремою доріжкою
                  </p>
                )}
              </div>
            ) : (
          ...
          >
            ...
            <div className="flex items-center gap-3 tabular-nums">
              ...
              {hasSel && (
                <span>
                  вид.:{' '}
                  {fmt((selection.end - selection.start) / TARGET_SAMPLE_RATE)}
                </span>
              )}
              <span>
                {clips.length} {clips.length === 1 ? 'доріжка' : 'доріжок'} ·{' '}
                {fmt(dur)}
              </span>
            </div>
          </div>
